...
class DistDecisionMakingNode(Node):
    ...
    def pedestrian_intention_state_list_callback(self, ped_list: PedestrianIntentionStateList) -> None:
        ...
        if num_pedestrians == 0:
            ...
            if np.linalg.norm(v_ped_1) > 0:
                if distance_veh_to_cross <= 30 and np.dot(v_ped_1, r_ped_1) < 0:
                    if 5 < distance_ped_to_cross <= 10 or 0.6 > intention_value >= 0.5:  # Near zone
                        self.Index = 1
    ...
    def simple_controller_and_publish(self):
        ...
        # Define control parameters based on Index --> Far 0 + Near zone 1 + Collision zone 2 + Passed zone 3 
        if self.Index == 0:  # Far zone
            base_speed = 8.0 
            Kp = 0.4 
            Ki = 0.1 
